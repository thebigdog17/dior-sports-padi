...
function getStatus(s){
  ...
  if(["IN_PLAY","INPROGRESS","IN PROGRESS","1H","2H","ET","LIVE"].some(x=>u.includes(x)))return{label:"LIVE",color:C.live,live:true,done:false};
  if(u==="HT"||u==="HALF TIME")return{label:"HT",color:C.gold,live:false,done:false};
  if(["FT","AET","PEN","FINISHED","MATCH FINISHED","FULL TIME"].some(x=>u.includes(x)))return{label:"FT",color:C.gray,live:false,done:true};
  if(u.includes("POSTP")||u.includes("CANCEL"))return{label:"PST",color:C.red,live:false,done:false};
...
function MatchRow({m,onClick,selected}){
  const home=m.home_name||m.strHomeTeam||"Home";
  const away=m.away_name||m.strAwayTeam||"Away";
  const hs=m.home_score!=null?m.home_score:null;
  const as_=m.away_score!=null?m.away_score:null;
  const st=getStatus(m.status||"");
  const logo=m.league_logo||"";
  return(
    <div onClick={onClick} style={{background:selected?C.peachLight:C.white,borderRadius:12,padding:"13px 16px",cursor:"pointer",transition:"all .15s",border:`1.5px solid ${selected?C.blue:st.live?C.live+"55":C.bluePale}`,borderLeft:`4px solid ${st.live?C.live:st.done?C.bluePale:C.peachMid}`,boxShadow:st.live?"0 2px 12px rgba(229,62,62,.14)":"none"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{minWidth:46,textAlign:"center"}}>
          {st.live?<div style={{background:C.live,color:"#fff",borderRadius:6,padding:"2px 7px",fontSize:10,fontWeight:800,display:"inline-block",animation:"lb 1.5s ease-in-out infinite"}}>LIVE</div>
            :<><div style={{fontSize:12,fontWeight:700,color:st.done?C.gray:C.blue}}>{st.label}</div>{m.time&&!st.done&&<div style={{fontSize:10,color:C.gray,marginTop:1}}>{m.time}</div>}</>}
        </div>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
            ...
            <span style={{fontWeight:900,fontSize:18,color:hs!=null?C.blue:C.gray,minWidth:24,textAlign:"right"}}>{hs!=null?hs:""}</span>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <span style={{fontWeight:900,fontSize:18,color:as_!=null?C.blue:C.gray,minWidth:24,textAlign:"right"}}>{as_!=null?as_:""}</span>
          </div>
        </div>
        {logo&&<img src={logo} alt="" style={{width:22,height:22,objectFit:"contain",opacity:.7,flexShrink:0}} onError={e=>e.target.style.display="none"}/>}
      </div>
    </div>
  );
}
...
function LiveScores(){
  ...
  const load=useCallback(async(quiet=false)=>{
    ...
    try{
      ...
      setMatches(fixtures);
      if(!fixtures.length&&!quiet)setErr("No fixtures found for "+fmtDate(date)+". Try a different date.");
      else if(fixtures.length)setErr("");
    }catch(e){if(!quiet)setErr("Could not load scores. Check your connection.");}
  ...
  async function openMatch(m){
    ...
    try{
      const r=await API.askAI(`Football: ${m.home_name} vs ${m.away_name} (${m.league_name}). Current score if available, last 3 results each team, top bet. MAX 7 lines.`);
      setDetail(r.result);
    }catch{setDetail("AI unavailable right now.");}
  ...
  const grouped={};
  shown.forEach(m=>{
    const k=m.league_name||m.country||"Other";
  ...
  return(
    <div>
      ...
      {!loading&&!err&&!matches.length&&<Card s={{textAlign:"center",padding:40}}>
        ...
        <div style={{color:C.gray,fontSize:13,marginTop:6}}>Monday is usually a quiet day — try Sat/Sun or Tue/Wed</div>
      </Card>}
      {Object.entries(grouped).map(([league,ms])=>(
        <div key={league} style={{marginBottom:18}}>
          <div style={{padding:"7px 14px",background:`linear-gradient(90deg,${C.dark},${C.blue})`,borderRadius:10,marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
            {ms[0]?.league_logo&&<img src={ms[0].league_logo} alt="" style={{width:18,height:18,objectFit:"contain"}} onError={e=>e.target.style.display="none"}/>}
            ...
            {ms.map((m,i)=>{
              ...
              return(<div key={id}>
                ...
                {selected===id&&<div style={{background:C.peachLight,borderRadius:"0 0 12px 12px",padding:"12px 16px",border:`1.5px solid ${C.blue}`,borderTop:"none",marginTop:-4}}>
                  {detailLoading?<div style={{fontSize:12,color:C.gray}}>⏳ Loading AI analysis...</div>
...
function AnalysisTab({user,onAuthRequired}){
  ...
  const loadFixtures=useCallback(async()=>{
    ...
    try{
      ...
      const fixtures=res?.fixtures||[];
      setMatches(fixtures);
      if(!fixtures.length)setErr("No fixtures for "+fmtDate(date)+". Try Tue/Wed or weekend dates.");
    }catch(e){setErr("Failed to load fixtures: "+e.message);}
    setLoading(false);
  },[date]);
  ...
  async function analyseMatch(m){
    ...
    try{
      const r=await API.askAI(`Analyse ${m.home_name} vs ${m.away_name} (${m.league_name}, ${date}). Markets: ${mktLabels}. For each: [Market]: [Pick] — [reason]. ⭐ confidence 1-5.`);
  ...
  async function buildTicket(){
    ...
    const matchList=top.map(m=>`• ${m.home_name} vs ${m.away_name} (${m.league_name||""})`).join("\n");
    const mktLabels=markets.map(id=>MARKETS.find(x=>x.id===id)?.label).filter(Boolean).join(", ");
    try{
  ...
  const grouped={};
  matches.forEach(m=>{const k=m.league_name||m.country||"Other";if(!grouped[k])grouped[k]=[];grouped[k].push(m);});
    ...
    <div>
            ...
            {ms.map((m,i)=>{
              ...
              return(<div key={id}>
                ...
                {expanded===id&&<div style={{background:C.peachLight,borderRadius:"0 0 12px 12px",padding:"14px 16px",border:`1.5px solid ${C.blue}`,borderTop:"none",marginTop:-4}}>
                  {matchLoading[id]?<div style={{fontSize:12,color:C.gray}}>⏳ Analysing...</div>
                    :<div style={{fontSize:13,lineHeight:1.8,color:C.dark,whiteSpace:"pre-wrap"}}>{matchAI[id]}</div>}
                </div>}
              </div>);
            })}
          </div>
      ...
      {ticket&&<div style={{borderLeft:`5px solid ${C.gold}`,borderRadius:16,padding:20,marginTop:16,background:`linear-gradient(135deg,${C.white},${C.peachLight}88)`,border:`1.5px solid ${C.bluePale}`}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14,flexWrap:"wrap"}}>
          ...
          <div><div style={{fontWeight:900,color:C.blue,fontSize:15}}>Smart Ticket — {fmtDate(date)}</div></div>
...
// ── LIVE TV — SportSRC (free, real streams) ───────────────────────
// SportSRC provides real multi-source stream embeds for live matches
const SPORTSRC = "https://sportsrc.org/api";
...
function LiveTV({user,onAuthRequired}){
  ...
  // Load live matches from SportSRC
  useEffect(()=>{
    if(!user)return;
    setLoading(true);
    fetch(`${SPORTSRC}/matches?sport=football&status=live`)
      .then(r=>r.ok?r.json():null)
      .then(d=>{
        if(d?.matches?.length){setMatches(d.matches);}
        else {
          // Fallback: show today's matches from our backend
          API.getLiveScores().then(res=>{
            setMatches((res?.fixtures||[]).slice(0,10));
          }).catch(()=>{});
        }
      })
      .catch(()=>{
  ...
  async function openMatch(m){
    if(!user){onAuthRequired();return;}
    setActive(m);setStreams([]);setStreamIdx(0);setStreamLoading(true);
    // Try SportSRC stream endpoint
    try{
      const matchId=m.id||m.match_id||m.event_id;
      const res=await fetch(`${SPORTSRC}/match/${matchId}/streams`);
      if(res.ok){
        const d=await res.json();
        if(d?.streams?.length){setStreams(d.streams);setStreamLoading(false);return;}
      }
    }catch{}
    // Fallback streams
    setStreams([
      {label:"Server 1 HD",url:`${SPORTSRC}/embed/${m.id||"live"}/1`},
      {label:"Server 2",url:`${SPORTSRC}/embed/${m.id||"live"}/2`},
      {label:"Server 3 SD",url:`${SPORTSRC}/embed/${m.id||"live"}/3`},
    ]);
  ...
  if(active)return(
    <div>
      ...
      <div style={{fontWeight:800,color:C.blue,marginBottom:10,fontSize:14}}>
        ⚽ {active.home_name||active.strHomeTeam||"Home"} vs {active.away_name||active.strAwayTeam||"Away"}
      </div>
      {/* Stream player */}
      <div style={{background:C.dark,borderRadius:18,overflow:"hidden",aspectRatio:"16/9",position:"relative",marginBottom:14}}>
        {streamLoading?<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}><Spin/></div>
          :streams.length>0?<iframe src={streams[streamIdx]?.url||streams[streamIdx]?.embed} style={{position:"absolute",inset:0,width:"100%",height:"100%",border:"none"}} allowFullScreen allow="autoplay;encrypted-media" title="stream"/>
          :<div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
            <div style={{fontSize:28,marginBottom:10}}>📺</div>
            <div style={{color:"#fff",fontWeight:700,marginBottom:6}}>Stream loading...</div>
            <div style={{color:C.gray,fontSize:12,textAlign:"center",padding:"0 20px"}}>Live streams available when matches are in progress</div>
          </div>}
      ...
      {/* Server switcher */}
      {streams.length>0&&<Card s={{marginBottom:14}}>
        <div style={{fontWeight:800,color:C.blue,marginBottom:10,fontSize:13}}>📡 Switch Server</div>
        <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
          {streams.map((sv,i)=><button key={i} onClick={()=>setStreamIdx(i)} style={{background:streamIdx===i?C.blue:C.peachLight,color:streamIdx===i?"#fff":C.dark,border:`1.5px solid ${streamIdx===i?C.blue:C.bluePale}`,borderRadius:10,padding:"8px 16px",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>{sv.label||`Server ${i+1}`}</button>)}
        </div>
      </Card>}
      {/* Chat */}
    ...
    <div>
      <div style={{background:`linear-gradient(135deg,${C.dark},${C.blue})`,borderRadius:16,padding:20,marginBottom:18,display:"flex",alignItems:"center",gap:14}}>
        ...
        <div>
          ...
          <div style={{color:C.bluePale,fontSize:12,marginTop:2}}>Real live streams · Multiple servers</div>
        </div>
        {matches.length>0&&<div style={{marginLeft:"auto",background:C.live,color:"#fff",borderRadius:8,padding:"5px 12px",fontSize:11,fontWeight:800,animation:"lb 1.5s ease-in-out infinite"}}>{matches.length} LIVE</div>}
      </div>
      ...
      {user&&!loading&&matches.length===0&&<Card s={{textAlign:"center",padding:40}}>
        <div style={{fontSize:40,marginBottom:10}}>📺</div>
        <div style={{fontWeight:700,color:C.blue,marginBottom:6}}>No live matches right now</div>
        <div style={{color:C.gray,fontSize:13}}>Live streams appear here when matches are in progress. Check back on match days — Tue/Wed for UCL, weekends for leagues.</div>
      </Card>}

      {user&&matches.length>0&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
        {matches.map((m,i)=>{
          const home=m.home_name||m.strHomeTeam||"Home";
          const away=m.away_name||m.strAwayTeam||"Away";
          const league=m.league_name||m.strLeague||"";
          const hs=m.home_score!=null?m.home_score:"";
          const as_=m.away_score!=null?m.away_score:"";
          return(
            <div key={m.id||i} onClick={()=>openMatch(m)} style={{background:C.white,borderRadius:14,overflow:"hidden",boxShadow:"0 3px 16px rgba(26,58,107,.1)",cursor:"pointer",border:`1.5px solid ${C.live}44`,transition:"transform .15s"}}
              ...
              <div style={{background:`linear-gradient(90deg,${C.dark},${C.blue})`,padding:"8px 14px",display:"flex",alignItems:"center",gap:8}}>
                ...
                <span style={{fontSize:11,color:C.bluePale,fontWeight:600}}>{league}</span>
                <span style={{marginLeft:"auto",fontSize:11,color:C.peachMid,fontWeight:700}}>▶ Watch</span>
              </div>
              <div style={{padding:"14px 16px",display:"flex",alignItems:"center",gap:14}}>
                <div style={{flex:1}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                    <span style={{fontWeight:700,fontSize:14,color:C.dark}}>{home}</span>
                    <span style={{fontWeight:900,fontSize:20,color:C.blue,minWidth:24,textAlign:"right"}}>{hs}</span>
                  </div>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontWeight:700,fontSize:14,color:C.dark}}>{away}</span>
                    <span style={{fontWeight:900,fontSize:20,color:C.blue,minWidth:24,textAlign:"right"}}>{as_}</span>
                  </div>
                </div>
              </div>
            </div>
